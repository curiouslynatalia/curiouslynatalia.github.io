  ...
  return (tree) => {
    visit(tree, 'text', (node: Text, index, parent) => {
      ...
      matches.forEach(({ tag, start, end, hasPrefix }) => {
        ...
        const tagHtml = {
          type: 'html',
          value: `<a href="/posts/tag/${encodeURIComponent(tag)}" class="no-underline text-xs text-primary-700 dark:text-primary-200 bg-primary-100 dark:bg-primary-800 px-2 py-1 rounded-full border border-primary-200 dark:border-primary-700 transition-colors hover:bg-highlight-100 dark:hover:bg-highlight-800">#${tag}</a>`
        };
